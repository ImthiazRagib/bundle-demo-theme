...
(function () {
  ...
  /* ── State ──────────────────────────────────────────────── */
  const state = {
    bundleType: null,      // 'single' | 'double' | 'triple' | 'infinity'
    totalBelts: 1,
  ...
  /* ── SKU color/name lookup tables (for catalog-based variant ID resolution) ── */
  const STRAP_SKU_COLOR = {
    'strap-nero':    'NERO',
    'strap-marrone': 'MARRONE',
    'strap-cognac':  'COGNAC',
    'strap-cuoio':   'CUOIO',
    'strap-beige':   'BEIGE',
    'strap-rosso':   'ROSSO',
  ...
  /* ── Catalog (flat SKU → variantId, from bundle-builder-catalog.json) ── */
  let _bbCatalog = null;
  ...
  /* Load the flat SKU→variantId catalog (bundle-builder-catalog.json).
     Used by proceedToCheckout() to resolve variant IDs without any Liquid injection. */
  async function fetchCatalog() {
    ...
    try {
      const res = await fetch(url);
      if (res.ok) {
        _bbCatalog = await res.json();
        console.log('[BundleBuilder] Catalog loaded —', Object.keys(_bbCatalog).length, 'SKU');
      }
    } catch (e) {
      console.warn('[BundleBuilder] Catalog not loaded (sync needed):', e);
    }
  }
  ...
  function updateBeltPreview() {
     ...
     * Sets preview.src IMMEDIATELY (images are preloaded into browser cache),
     * then fades out → in for a smooth transition.
     * All async callbacks check _previewLoadId to cancel stale updates. */
    ...
    function showPreviewPhoto(url) {
      ...
      /* Set source immediately — browser reads from cache for preloaded images */
      preview.src = url;
      /* Step 1: fade out */
      preview.style.transition = 'opacity 0.12s ease-out';
      preview.style.opacity    = '0.15';
      const lid = _loadId;
      /* Step 2: after 60ms fade back in (enough time for fade-out to paint) */
      setTimeout(() => {
        if (_previewLoadId !== lid) return; /* cancelled by newer call */
        requestAnimationFrame(() => {
          if (_previewLoadId !== lid) return;
  ...
  function showReview() {
    ...
    const bundleLabel = (BUNDLES[state.bundleType] || BUNDLES.single).name;
    ...
    state.belts.forEach(function(belt, i) {
      ...
      html += '<div class="bb-review__card">'
        ...
        + '</div>'
        + '<button class="bb-review__card-edit" data-edit="' + (i + 1) + '">✏ Modifica</button>'
        + '</div></div>';
    });
    ...
    if (totalEl) totalEl.textContent = '€ ' + totalPrice.toFixed(2).replace('.', ',');
  ...
  function addBundleToCart() {
    const bundle = {
      ...
      price: parseFloat(((BUNDLES[state.bundleType] || BUNDLES.single).price + (state.bundleType === 'infinity' ? Math.max(0, state.totalBelts - 4) * 19.99 : 0)).toFixed(2)),
      qty: 1,
    };
    state.cartBundles.push(bundle);
    renderCart();
    showScreen('cart');
  }
  ...
  async function proceedToCheckout() {
    /*
     * Cart structure (3 layers):
     *
     *  1. BUNDLE product (1 visible line item, priced)
     *     Variant ID from window.BB_VARIANTS.bundles (Liquid-injected).
     *     Has "Composizione" property listing all belts — visible to customer.
     *
     *  2. COMPONENT SKUs per belt at €0 (strap, buckle, passante)
     *     Variant IDs resolved from _bbCatalog (bundle-builder-catalog.json).
     *     Properties prefixed with "_" are hidden from customer emails/receipt
     *     but fully visible in Shopify Admin orders and logistics apps.
     *
     *  3. LOGISTICS SKUs per belt at €0 (box, warranty card, NFC card)
     *     Same mechanism as components.
     *
     * Run "Sincronizza Catalogo" in the admin dashboard to populate
     * bundle-builder-catalog.json before going live.
     */
    const V       = window.BB_VARIANTS || {};
    const bundleV = V.bundles || {};
    const CAT     = _bbCatalog || {};   /* flat: "STRAP-NERO-130" → variantId */
    ...
    state.cartBundles.forEach(function(bundle) {
      ...
      /* ── 1. Bundle product (visible, priced) ─────────────── */
      ...
      if (bundleV[bundleVarKey]) {
        const compositionSummary = bundle.belts.map(function(b, idx) {
          ...
          return 'Cintura ' + (idx + 1) + ': ' + (b.length || '') +
                 ' · Fibbia ' + (k.name || '') + ' · Pelle ' + (s.name || '');
        }).join(' | ');
        items.push({
          id:         bundleV[bundleVarKey],
          quantity:   qty,
          properties: { 'Composizione': compositionSummary },
        });
      }

      /* ── 2. Infinity extra-belt product ──────────────────── */
      ...
      /* ── 3. Component + logistics SKUs per belt ──────────── */
      bundle.belts.forEach(function(belt, i) {
        const beltLabel = 'Cintura ' + (i + 1);
        const lengthNum = (belt.length || '130cm').replace('cm', '').trim(); /* "130" | "150" */
        const skuColor  = STRAP_SKU_COLOR[belt.strap]    || belt.strap.replace('strap-', '').toUpperCase();
        const skuBuckle = BUCKLE_SKU_NAME[belt.buckle]   || belt.buckle.replace('buckle-', '');

        const strapVarId    = CAT['STRAP-'    + skuColor + '-' + lengthNum];
        const buckleVarId   = CAT['FIBBIA-'   + skuBuckle];
        const passanteVarId = CAT['PASSANTE-' + skuColor];
        const boxVarId      = CAT['BOX-CINTURA'];
        const warrantyVarId = CAT['WARRANTY-CARD'];
        const nfcVarId      = CAT['NFC-CARD'];
        ...
        if (strapVarId) {
          items.push({
            id: strapVarId, quantity: qty,
            properties: {
              '_cintura':  beltLabel,
              '_pos':      String(i + 1),
              'Pelle':     strapInfo.name || skuColor,
        ...
        if (buckleVarId) {
          items.push({
            id: buckleVarId, quantity: qty,
            properties: {
              '_cintura': beltLabel,
              '_pos':     String(i + 1),
              'Modello':  buckleInfo.name || skuBuckle,
              '_bundle':  bundle.type,
            },
          });
        }
        ...
        if (passanteVarId) {
          items.push({
            id: passanteVarId, quantity: qty,
            properties: {
              '_cintura': beltLabel,
              '_pos':     String(i + 1),
              'Colore':   strapInfo.name || skuColor,
              '_bundle':  bundle.type,
            },
          });
        }

        /* Logistica: box, garanzia, NFC — nascosti al cliente,
           visibili in Shopify Admin e nelle app di picking. */
        [
          { id: boxVarId,      label: 'Belt Box',      sku: 'BOX-CINTURA'   },
          { id: warrantyVarId, label: 'Warranty Card',  sku: 'WARRANTY-CARD' },
          { id: nfcVarId,      label: 'NFC Card',       sku: 'NFC-CARD'      },
    ...
    /* Filter out items without a resolved variant ID.
       Shopify rejects the entire /cart/add.js call with 422 if any ID is 0 or falsy.
       If catalog is not yet synced, component items are simply omitted and only
       the visible bundle product is added (graceful degradation). */
    const validItems = items.filter(function(item) { return item.id && item.id !== 0; });

    /* If running outside Shopify (preview/demo), simulate success */
    if (typeof window.Shopify === 'undefined' && !window.location.hostname.includes('myshopify')) {
      showConfirmed();
      return;
    }

    /* No items at all — bundle products not yet configured in Theme Editor */
    if (!validItems.length) {
      showToast('Configura i prodotti bundle nel Theme Editor per abilitare il checkout.');
      return;
    }
    ...
    try {
      ...
      if (resp.ok) {
        /* Redirect to Shopify standard checkout — exits the bundle builder completely */
      ...
      /* Network error or non-Shopify environment */
      showConfirmed();
    }
  }
  ...
  async function init() {
    ...
    /* Load per-combination media config + component SKU catalog */
